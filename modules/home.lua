...
local webhook12 = "https://discordapp.com/api/webhooks/663105403466285076/XuOypVgTMPwoPTF3FGdp9Y3a6DgK_SjzDQRn-IXRUVRHtWEN31FbMkNObX5B4wSRPvfB" -- بيع البيوت

function sendWebhookMessage(webhook12,message)
	if webhook12 ~= "none" then
		PerformHttpRequest(webhook12, function(err, text, headers) end, 'POST', json.encode({content = message}), { ['Content-Type'] = 'application/json' })
	end
end
...
local function build_entry_menu(user_id, home_name)
  ...
  menu[lang.home.sell.title()] = {function(player,choice)
    vRP.prompt(player,"متاكد تبي تبيع؟ اذا ايه اكتب بالمربع","",function(player,desc) -- وهذي
      desc = desc or "" -- وهذي
      if desc ~= nil and desc ~= "" then -- هذي
        vRP.getUserAddress(user_id, function(address)
          if address ~= nil and address.home == home_name then -- check if already have a home
			...
			vRPbm.logInfoToFile("sellhouselog.txt",user_id .. " got " .. home.sell_price .. " by selling ".. address.home .. "\r\n")
	     sendWebhookMessage(webhook12,"```xl\r\n User_id " .. user_id .." got " .. home.sell_price .." by selling "..  address.home .."```")
          else
