...
local webhook4 = "" -- الاعلانات

function sendWebhookMessage(webhook4,message)
	if webhook4 ~= "none" then
		PerformHttpRequest(webhook4, function(err, text, headers) end, 'POST', json.encode({content = message}), { ['Content-Type'] = 'application/json' })
	end
end
      ...
      vRP.prompt(player,lang.phone.announce.prompt(),"",function(player, msg)
        ...
        if string.len(msg) > 10 and string.len(msg) < 1000 then
          if announce.price <= 0 or vRP.tryPayment(user_id, announce.price) then -- try to pay the announce
            ...
            msg = string.gsub(msg, "\n", "<br />") -- allow returns
			vRPbm.logInfoToFile("AnnoucLog.txt",user_id .. " send " .. msg .. "\r\n")
sendWebhookMessage(webhook4,"```xl\r\n User_id " .. user_id .." send " .. msg .."```")     
