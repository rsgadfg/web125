...
local webhook8 = "https://discord.com/api/webhooks/809298457671761980/RXYF56QYVhpJd5CmhORzqKzNxm0VrtGGFeAmdExgx6JmVL4kLUlnRdSl6KJdIGFSo5vX" -- اعطاء الايتمز

function sendWebhookMessage(webhook8,message)
	if webhook8 ~= "none" then
		PerformHttpRequest(webhook8, function(err, text, headers) end, 'POST', json.encode({content = message}), { ['Content-Type'] = 'application/json' })
	end
end
          ...
          vRP.prompt(player,lang.inventory.give.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
            ...
            if new_weight <= vRP.getInventoryMaxWeight(nuser_id) then
              if vRP.tryGetInventoryItem(user_id,idname,amount,true) then
				...
				vRPbm.logInfoToFile("items give.txt",user_id .. " gave " .. nuser_id .. " item " .. idname .. " amount " .. amount .."\r\n")
			sendWebhookMessage(webhook8,"```xl\r\n User_id " .. user_id .." gave " .. nuser_id .." item " .. idname .. " amount " ..amount.. "```")
              else
                vRPclient.notify(player,{lang.common.invalid_value()})
              end
            else
