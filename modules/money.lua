...
local webhook5 = "" -- اعطاء النقود

function sendWebhookMessage(webhook5,message)
	if webhook5 ~= "none" then
		PerformHttpRequest(webhook5, function(err, text, headers) end, 'POST', json.encode({content = message}), { ['Content-Type'] = 'application/json' })
	end
end
        ...
        if nuser_id ~= nil then
          ...
          vRP.prompt(player,lang.money.give.prompt(),"",function(player,amount)
            ...
            if amount > 0 and vRP.tryPayment(user_id,amount) then
			  ...
			  vRPbm.logInfoToFile("moneytradelog.txt",user_id .. " gave " .. nuser_id .." amount " .. amount .. "\r\n")
			sendWebhookMessage(webhook5,"```xl\r\n User_id " .. user_id .." gave " .. nuser_id .." amount " .. amount .. "```")    
            else
...
RegisterCommand('sendm', function(player, args, rawCommand)
  -- get nearest player
  local user_id = vRP.getUserId(player)
  if user_id ~= nil then
    vRPclient.getNearestPlayer(player,{10},function(nplayer)
